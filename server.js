...
html += `<tr><td>${row.const http = require('http');
// 1. เรียกใช้งาน Pool จากไลบรารี pg สําหรับจัดการการเชื่อมต่อฐานขอมูล
const { Pool } = require('pg');
// 2. ตั้งคาการเชื่อมตอ โดยดึง URL มาจาก Environment Variable ของ Railway
const pool = new Pool({
connectionString: process.env.DATABASE_URL,
...
// วนลูปนําขอมูลแตละแถวมาแสดง
result.rows.forEach(row => {
html += `<tr><td>${row.students_id}</td><td>${row.student_name}</td></tr>`;
});
result.rows.forEach(row => {
html += `<tr><td>${row.students_id}</td><td>${row.student_name}</td></tr>`;
});
